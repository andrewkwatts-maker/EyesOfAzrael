...
/**
 * Firebase Link Validation and Repair Script v2
 * Eyes of Azrael
 *
 * Validates and fixes broken internal links in Firebase entity assets.
 * Updated to handle object-based link references with name/path fields.
 *
 * Features:
 ...
 * - Converts path-based and name-based references to proper ID references
 * - Generates fix script and validation report
 * - Batch updates with verification
 *
 * Usage:
 *   node scripts/validate-fix-links-v2.js --scan        # Scan only, no fixes
 *   node scripts/validate-fix-links-v2.js --fix         # Scan and apply fixes
 *   node scripts/validate-fix-links-v2.js --mythology greek  # Specific mythology
 *   node scripts/validate-fix-links-v2.js --report      # Generate detailed report
 */
...
function extractIdFromPath(pathStr) {
  ...
  let cleanPath = pathStr.replace(/\.html$/, '');
  ...
  for (const filePath of jsonFiles) {
    try {
      ...
      if (!data.id || !data.name) {
        console.warn(`   ⚠️  Skipping ${path.basename(filePath)}: missing id or name`);
        continue;
      }
      ...
      // Add path variations
      const pathVariations = generatePathVariations(data.id, entityType, mythology);
      pathVariations.forEach(pathVar => {
        index.byPath[normalizeString(pathVar)] = data.id;
      });
...
/**
 * Generate path variations for entity
 */
function generatePathVariations(id, type, mythology) {
  const variations = [];

  // Common path patterns
  const pluralType = type + 's';

  variations.push(`${mythology}/${pluralType}/${id}`);
  variations.push(`${mythology}/${type}/${id}`);
  variations.push(`mythos/${mythology}/${pluralType}/${id}`);
  variations.push(`mythos/${mythology}/${type}/${id}`);
  variations.push(`data/entities/${type}/${id}`);
  variations.push(`data/entities/${mythology}/${pluralType}/${id}`);
...
function validateLink(linkRef, entityIndex, context = {}) {
  ...
  let entityId;
  let linkFormat;
  let originalLink = linkRef;

  // Handle different link formats
  if (typeof linkRef === 'string') {
    // Simple string ID or path
    if (linkRef.includes('/')) {
      entityId = extractIdFromPath(linkRef);
      linkFormat = 'path_string';
    } else {
      entityId = linkRef;
      linkFormat = 'id_string';
    }
  } else if (typeof linkRef === 'object') {
    // Object with various possible fields
    if (linkRef.id) {
      entityId = linkRef.id;
      linkFormat = 'object_with_id';
    } else if (linkRef.path) {
      entityId = extractIdFromPath(linkRef.path);
      linkFormat = 'object_with_path';
    } else if (linkRef.url) {
      entityId = extractIdFromPath(linkRef.url);
      linkFormat = 'object_with_url';
    } else if (linkRef.name) {
      // Try to find by name
      const normalizedName = normalizeString(linkRef.name);
      const candidates = entityIndex.byName[normalizedName];

      if (candidates && candidates.length === 1) {
        entityId = candidates[0];
        linkFormat = 'object_with_name';
      } else if (candidates && candidates.length > 1) {
        // Multiple matches - try to narrow by mythology/type
        const mythology = linkRef.mythology || linkRef.tradition || context.mythology;
        const filtered = candidates.filter(id => {
          const entity = entityIndex.byId[id];
          return !mythology || entity.mythologies.includes(mythology);
        });

        if (filtered.length === 1) {
          entityId = filtered[0];
          linkFormat = 'object_with_name_filtered';
        } else {
          // Still ambiguous
          return {
            valid: false,
            ...
            original: linkRef,
            candidates: filtered.map(id => entityIndex.byId[id])
          };
        }
      } else {
        entityId = null;
        linkFormat = 'object_with_name_not_found';
      }
    } else {
      return {
        valid: false,
        issue: 'invalid_object_format',
        original: linkRef
      };
    }
  } else {
  ...
  if (entityId && entityIndex.byId[entityId]) {
    return {
      valid: true,
      entityId: entityId,
      format: linkFormat,
      entity: entityIndex.byId[entityId],
      needsConversion: linkFormat !== 'id_string' && linkFormat !== 'object_with_id'
    };
  }

  // Try fuzzy matching
  const searchTerm = entityId || (linkRef.name ? linkRef.name : String(linkRef));
  const suggestions = findSimilarEntities(searchTerm, entityIndex, context);
 ...
 */
function findSimilarEntities(searchTerm, entityIndex, context = {}) {
  ...
  for (const [id, entity] of Object.entries(entityIndex.byId)) {
    ...
    const idSimilarity = calculateSimilarity(searchTerm, id);
...
function generateFixes(validationResults, entityIndex) {
  ...
  for (const result of validationResults) {
    const entityFixes = {
      ...
      entityFile: result.entityFile,
      fixes: [],
      conversions: result.conversions || []
    };

    // Count conversions
    conversions += result.conversions ? result.conversions.length : 0;
    ...
    if (entityFixes.fixes.length > 0 || entityFixes.conversions.length > 0) {
      fixes.push(entityFixes);
    }
  }

  console.log(`   ✅ Auto-fixable: ${autoFixable}`);
  console.log(`   🔄 Valid but needs conversion: ${conversions}`);
...
function applyFixes(fixes, dryRun = true) {
  ...
  for (const entityFix of fixes) {
    ...
    try {
      ...
      // Apply conversions (valid links that need format conversion)
      for (const conversion of entityFix.conversions) {
        const field = conversion.field;
        let value = entity[field];

        if (Array.isArray(value) && value[conversion.index]) {
          // Convert to simple ID string
          value[conversion.index] = conversion.newId;
          stats.linksConverted++;
          modified = true;
        }
      }

      // Apply fixes (broken links)
...
function generateReport(validationResults, fixes, stats) {
  ...
  const totalConversions = validationResults.reduce((sum, r) => sum + (r.needsConversion || 0), 0);
  ...
  const report = {
    ...
    summary: {
      ...
      needsConversion: totalConversions,
      entitiesWithIssues: validationResults.filter(r => r.brokenLinks > 0).length,
      entitiesNeedingConversion: validationResults.filter(r => (r.needsConversion || 0) > 0).length
    },
    issuesByType: {},
    ...
    validationResults: validationResults.filter(r => r.brokenLinks > 0 || (r.needsConversion || 0) > 0)
  };
...
function generateMarkdownSummary(report) {
  ...
  lines.push('# Firebase Link Validation Report v2');
  lines.push('');
  ...
  lines.push(`- **Links Needing Format Conversion:** ${report.summary.needsConversion.toLocaleString()}`);
  ...
  lines.push(`- **Entities Needing Conversion:** ${report.summary.entitiesNeedingConversion.toLocaleString()}`);
  lines.push('');
  ...
  lines.push(`- **Format Conversions (valid but wrong format):** ${report.fixes.conversions.toLocaleString()}`);
  lines.push(`- **Needs Manual Review:** ${report.fixes.needsReview.toLocaleString()}`);
  lines.push('');

  if (report.topProblematicEntities.length > 0) {
  ...
  lines.push('2. Run with `--fix` flag to apply auto-fixable changes and format conversions');
...
function scanLinks(options = {}) {
  ...
  for (const filePath of jsonFiles) {
    try {
      ...
      // Add filepath to entity for later use
      ...
      console.warn(`\n   ⚠️  Error scanning ${path.basename(filePath)}: ${error.message}`);
    }
  }
...
async function main() {
  ...
  if (options.help) {
    console.log(`
Firebase Link Validation Script v2
${'='.repeat(80)}
...
Usage:
  node scripts/validate-fix-links-v2.js [options]

Options:
  --scan              Scan only, no fixes (default)
...
Examples:
  ...
  # Scan and fix (dry run to see what would change)
  ...
  node scripts/validate-fix-links-v2.js --report
    `);
    return;
  }
  ...
  if (options.fix) {
    ...
    console.log(`   Converted: ${applyStats.linksConverted} links (format conversion)`);
    console.log(`   Fixed: ${applyStats.linksFixed} broken links`);
    console.log(`   Removed: ${applyStats.linksRemoved} invalid links`);
  ...
  console.log(`Links Needing Conversion: ${report.summary.needsConversion} (${((report.summary.needsConversion / report.summary.totalLinks) * 100).toFixed(1)}%)`);
  console.log(`Broken Links: ${report.summary.brokenLinks} (${((report.summary.brokenLinks / report.summary.totalLinks) * 100).toFixed(1)}%)`);
  console.log(`\nEntities with Issues: ${report.summary.entitiesWithIssues}`);
  console.log(`Entities Needing Conversion: ${report.summary.entitiesNeedingConversion}`);
  console.log('\nFixes:');
  console.log(`  Auto-fixable: ${report.fixes.autoFixable}`);
  console.log(`  Format conversions: ${report.fixes.conversions}`);
