/**
 * Home Page View - FALLBACK ONLY VERSION
 * For testing and debugging - ALWAYS uses fallback mythologies
 * This version bypasses Firebase entirely
 */

class HomeViewFallbackOnly {
    constructor() {
        // No firestore needed - pure fallback mode
        this.mythologies = [];
    }

    /**
     * Render the home page using ONLY fallback data
     */
    async render(container) {
        console.log('[Home View Fallback] Rendering home page (FALLBACK MODE ONLY)...');
        ...
        container.innerHTML = `
            <div class="loading-container">
                ...
                <p class="loading-message">Loading mythologies (fallback mode)...</p>
            </div>
        `;

        try {
            // Load fallback mythologies (no Firebase)
            this.loadFallbackMythologies();
            ...
            console.log(`[Home View Fallback] Successfully rendered ${this.mythologies.length} mythology cards`);

        } catch (error) {
            console.error('[Home View Fallback] Error rendering home page:', error);
            container.innerHTML = this.getErrorHTML(error);
        }
    }

    /**
     * Load fallback mythologies (no Firebase call)
     */
    loadFallbackMythologies() {
        console.log('[Home View Fallback] Loading fallback mythologies...');
        this.mythologies = this.getFallbackMythologies();
        console.log(`[Home View Fallback] Loaded ${this.mythologies.length} fallback mythologies`);
    }
    ...
    attachEventListeners() {
        ...
        console.log(`[Home View Fallback] Attaching event listeners to ${cards.length} cards`);

        cards.forEach(card => {
            card.addEventListener('mouseenter', () => {
                console.log('[Home View Fallback] Hovering over:', card.dataset.mythology);
            });
        });
    }
}
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = HomeViewFallbackOnly;
}
