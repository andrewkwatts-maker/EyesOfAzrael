...
class AdvancedSearchSystem {
    ...
    /**
     * Every entity collection this page can search.
     *
     * The same hardcoded-list idiom js/mythology-comparisons.js uses. The
     * alternative — listCollections() — is an admin-only call that a browser
     * client cannot make, so the list lives here.
     */
    static SEARCH_COLLECTIONS = [
        'deities', 'heroes', 'creatures', 'places', 'items', 'texts',
        'concepts', 'symbols', 'rituals', 'herbs', 'archetypes',
        'magic_systems', 'cosmology', 'events', 'myths'
    ];

    /**
     * Build search index from Firestore.
     *
     * This used to read collection('content').where('status','==','published').
     * There is no 'content' collection in the project — the data is split across
     * per-type collections (deities, heroes, …) — and no document anywhere
     * carries a 'status' field, so the filter would have matched nothing even if
     * the collection had existed.
     *
     * That failure was silent. Firestore returns an empty snapshot rather than
     * an error for a missing collection, so the page built an index of zero
     * entries, logged "Search index built with 0 entries", and reported "no
     * results" for every query — indistinguishable from a genuine empty search.
     */
    async buildSearchIndex() {
        console.log('Building search index...');

        const perCollectionLimit = Math.ceil(1000 / AdvancedSearchSystem.SEARCH_COLLECTIONS.length);

        const snapshots = await Promise.all(
            AdvancedSearchSystem.SEARCH_COLLECTIONS.map(async (name) => {
                try {
                    const snap = await this.db.collection(name).limit(perCollectionLimit).get();
                    return { name, docs: snap.docs };
                } catch (error) {
                    // One unreadable collection must not empty the whole index.
                    console.warn(`Search index: skipped "${name}" — ${error.message}`);
                    return { name, docs: [] };
                }
            })
        );
        ...
        for (const { name, docs } of snapshots) {
            for (const doc of docs) {
                this.indexContent({ id: doc.id, contentType: name, ...doc.data() });
            }
        }

        if (this.searchIndex.size === 0) {
            // Say so loudly rather than silently serving an empty index.
            console.error('Search index is EMPTY — every query will return no results.');
        } else {
    ...
    /**
     * Normalise a stored document to the field names this class searches on.
     *
     * The live collections name things differently from the retired 'content'
     * collection this file was written against: name/description/type there,
     * title/summary/contentType here. Mapping once on the way in keeps the
     * rest of the class — scoring, filtering, rendering, suggestions —
     * untouched.
     */
    static normalizeDocument(content) {
        return {
            ...content,
            title: content.title || content.name || content.displayName || '',
    ...
    indexContent(rawContent) {
        const content = AdvancedSearchSystem.normalizeDocument(rawContent);
