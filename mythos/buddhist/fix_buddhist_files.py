...
class BuddhistFileFixer:
    ...
    def fix_corrupted_html(self, content: str) -> Tuple[str, int]:
        ...
        # Pattern 1: Corrupted titles with nested <a> tags
        # Extract clean text from mangled titles
        title_pattern = r'<title>(?:<a[^>]*>)*([A-Za-z\s]+)(?:</a>)*\s*-\s*Buddhist\s*(?:Mythology|Concepts|Deities)?</title>'
        ...
        content = re.sub(title_pattern, clean_title, content, flags=re.DOTALL | re.IGNORECASE)

        # Pattern 2: Simple corrupted corpus-link patterns in text
        # Remove nested corpus-link anchors - keep only the innermost text
        corpus_link_pattern = r'<a[^>]*class="corpus-link"[^>]*>(?:<a[^>]*>)*([^<]+)(?:</a>)*</a>'

        def clean_corpus_link(match):
            nonlocal fixes
        ...
        for _ in range(5):
            new_content = re.sub(corpus_link_pattern, clean_corpus_link, content, flags=re.DOTALL)
    ...
    def fix_corpus_links(self, content: str, file_path: Path) -> Tuple[str, int]:
        ...
        def replace_link(match):
            ...
            term_display = term.replace('_', ' ').replace('-', ' ').title()
            fixes += 1
...
if __name__ == "__main__":
    ...
    report_path = Path(base_dir) / "fix_report.txt"
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("Buddhist Files Fix Report\n")
        f.write("="*60 + "\n\n")
        for item in report:
